      ...
      quizQuestions.forEach(question => {
        ...
        ['A', 'B', 'C', 'D'].forEach(option => {
          if (!question.options[option as 'A' | 'B' | 'C' | 'D'].trim()) {
            newErrors[`${question.id}-${option}`] = `Option ${option} is required`;
          }
        });
        
        if (!question.correctAnswer) {
          newErrors[`${question.id}-correctAnswer`] = 'Please select the correct answer';
        }
      });
    }
        ...
        <div className="mb-6">
          <div className="bg-gray-50 p-4 border rounded-lg mb-4">
            ...
            <p className="text-sm text-gray-600">
              This is how students will see your quiz. Try answering the questions to test it.
            </p>
          </div>
              ...
              <div className="p-4">
                ...
                <div className="mt-6 space-y-4">
                  {quizQuestions.map((question, index) => {
                    const isCorrect = previewAnswers[question.id] === question.correctAnswer;
                    
                    return (
                      <div key={question.id} className="border rounded-lg p-4">
                        <div className="flex items-start">
                          <div className={`flex-shrink-0 rounded-full w-6 h-6 flex items-center justify-center mr-2 ${isCorrect ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                            {isCorrect ? (
                          ...
                          <div>
                            ...
                            <div className="mt-2 text-sm">
                              <p>Your answer: <span className="font-medium">{previewAnswers[question.id]}</span></p>
                              {!isCorrect && (
                          ...
                          {['A', 'B', 'C', 'D'].map((option) => (
                            ...
                            >
                              <input
                                ...
                                onChange={() => handlePreviewAnswer(question.id, option)}
                      ...
                      {(provided) => (
                        ...
                        >
                          <div className="bg-gray-50 p-3 border-b flex items-center">
                            ...
                            <h3 className="font-medium flex-1">Question {index + 1}</h3>
                            <button
                              type="button"
                          ...
                          <div className="p-4 space-y-4">
                            <div>
                              ...
                              {errors[`${question.id}-question`] && (
                                <p className="mt-1 text-sm text-red-600">{errors[`${question.id}-question`]}</p>
                              )}
                            </div>
                            ...
                            <div>
                                  ...
                                  <div key={option} className="flex items-center">
                                    <label className="flex items-center mr-3">
                                      <input
                                        ...
                                        className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                                      />
                                      <span className="ml-1 text-sm font-medium">{option}</span>
                                    </label>
                                    <input
                                      type="text"
                              ...
                              {errors[`${question.id}-correctAnswer`] && (
                                <p className="mt-2 text-sm text-red-600">{errors[`${question.id}-correctAnswer`]}</p>
                              )}
                              
                              {['A', 'B', 'C', 'D'].some(option => errors[`${question.id}-${option}`]) && (
                                <p className="mt-2 text-sm text-red-600">All options are required</p>
                              )}
                            </div>
                          </div>
                        </div>
